...
export const BrochureShowcase: React.FC = () => {
  ...
  const servicesAndPrograms = [
    {
      ...
      icon: <Users className="w-6 h-6 text-[#A64D32]" />,
      badge: 'Core Program'
    },
    {
      ...
      icon: <Sparkles className="w-6 h-6 text-[#5B6346]" />,
      badge: 'Annual Event'
    },
    {
      ...
      icon: <Heart className="w-6 h-6 text-[#A64D32]" />,
      badge: 'Community'
    },
    {
      ...
      icon: <BookOpen className="w-6 h-6 text-[#5B6346]" />,
      badge: 'Youth'
    },
    {
      ...
      icon: <ShieldCheck className="w-6 h-6 text-[#A64D32]" />,
      badge: 'Relief'
    },
    {
      ...
      icon: <Trophy className="w-6 h-6 text-[#5B6346]" />,
      badge: 'Sports'
    }
  ];
  ...
  return (
    <section className="py-20 bg-[#F4F1ED] text-[#2A2A2A] border-t border-b border-[#E5E0D8]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        
        {/* Section Header */}
        <AnimatedSection direction="up" delayMs={50}>
          <div className="text-center max-w-3xl mx-auto space-y-4">
            <div className="inline-flex items-center gap-2 text-[#A64D32] font-bold text-xs uppercase tracking-[0.2em] bg-[#A64D32]/10 px-4 py-1.5 rounded-full border border-[#A64D32]/20">
        ...
        <AnimatedSection direction="up" delayMs={100}>
          <div className="flex justify-center border-b border-[#E5E0D8] pb-4 gap-2 sm:gap-4 overflow-x-auto">
            <button
              onClick={() => setActiveTab('signature')}
              className={`px-6 py-3 rounded-full text-xs font-bold uppercase tracking-widest transition-all cursor-pointer ${
                activeTab === 'signature'
                  ? 'bg-[#A64D32] text-white shadow-md'
              ...
              <AnimatedSection key={index} direction="up" delayMs={100 + (index % 3) * 100}>
                <div className="bg-[#FDFBF7] rounded-[24px] overflow-hidden border border-[#E5E0D8] shadow-sm hover:shadow-md transition-all group flex flex-col justify-between h-full">
                  ...
                  <div className="relative h-48 overflow-hidden bg-gradient-to-br from-[#5B6346] to-[#A64D32]">
                    ...
                    <span className="absolute top-4 left-4 bg-[#5B6346] text-white px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider shadow-sm">
                      {act.category}
                    </span>
                  </div>
                  ...
                  <div className="p-6 space-y-3 flex-1 flex flex-col justify-between">
                    <div>
                      <h3 className="text-xl font-serif font-bold text-[#2A2A2A] group-hover:text-[#A64D32] transition-colors">
                        {act.title}
                      </h3>
        ...
        {activeTab === 'programs' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {servicesAndPrograms.map((prog, index) => (
              <AnimatedSection key={index} direction="up" delayMs={100 + (index % 3) * 100}>
                <div className="bg-[#FDFBF7] rounded-[24px] p-8 border border-[#E5E0D8] shadow-sm hover:shadow-md transition-all space-y-4 group h-full">
                  <div className="flex items-center justify-between">
                    <div className="w-12 h-12 rounded-2xl bg-[#F4F1ED] flex items-center justify-center border border-[#E5E0D8]">
                      {prog.icon}
                    </div>
                    <span className="text-[10px] font-bold uppercase tracking-widest text-[#A64D32] bg-[#A64D32]/10 px-3 py-1 rounded-full">
                      {prog.badge}
                    </span>
                  </div>
        ...
        {activeTab === 'vision' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            {/* Our Mission */}
            <AnimatedSection direction="up" delayMs={100}>
              <div className="bg-[#FDFBF7] rounded-[28px] p-8 sm:p-10 border border-[#E5E0D8] space-y-6 shadow-sm border-l-4 border-l-[#A64D32] h-full">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-2xl bg-[#A64D32] text-white flex items-center justify-center font-serif font-bold text-xl">
                    M
                  </div>
                  <div>
                ...
                <p className="text-[#5A5A5A] text-base leading-relaxed font-sans">
                  {isEs 
                ...
                <div className="p-4 bg-[#F4F1ED] rounded-xl border border-[#E5E0D8] text-xs font-bold text-[#2A2A2A] uppercase tracking-wider flex items-center gap-2">
            ...
            <AnimatedSection direction="up" delayMs={200}>
              <div className="bg-[#FDFBF7] rounded-[28px] p-8 sm:p-10 border border-[#E5E0D8] space-y-6 shadow-sm border-l-4 border-l-[#5B6346] h-full">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-2xl bg-[#5B6346] text-white flex items-center justify-center font-serif font-bold text-xl">
                    V
                  </div>
                  <div>
                    <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#5B6346]">
                ...
                <div className="p-4 bg-[#F4F1ED] rounded-xl border border-[#E5E0D8] text-xs font-bold text-[#2A2A2A] uppercase tracking-wider flex items-center gap-2">
                  <Compass className="w-4 h-4 text-[#5B6346]" />
